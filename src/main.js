...
for (let [index, comp] of Object.entries(comps))
{
    ...
    document.getElementById("gen-comp" + index + "-amount").innerHTML = format(comp.amount) + ",";
}
...
function buyComp(comp)
{
    if (points.gte(comps[comp].cost))
    {
        ...
        comps[comp].amount = comps[comp].amount.add(1);
        
        //update cost, formula: 10^(4(comp) + 2x(comp) - 3), x = comps[comp].bought
        let temp = new Decimal((comp * 4) - 3).add(new Decimal(comp)
                    .mul(2).mul(comps[comp].bought));
...
{
    return comps[comp].amount.pow(compExp);
}
...
function calcCompxPerSecond(comp) 
{
    ...
    return getTrueAmount(comp).mul(comps[comp + 1].multi);
}
